...
import { CreditCard, QrCode, Clock, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
...
const VendorDashboardHome = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: payments } = await supabase.from("payments").select("*").eq("vendor_id", vendor?.id || "").order("created_at", { ascending: false }).limit(5);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between rounded-xl bg-primary px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-primary-foreground">Monthly stall fee</span>
        </div>
        <span className="font-mono text-sm font-bold text-primary-foreground">₱{Number(monthlyRate).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</span>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <motion.div whileHover={{ y: -2 }} className="rounded-2xl border bg-card p-6 shadow-civic">
          <span className="text-sm font-medium uppercase tracking-wider text-muted-foreground">Monthly Fee</span>
          <h2 className="mt-2 font-mono text-4xl font-bold text-foreground">₱{Number(monthlyRate).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</h2>
          <Link to="/vendor/pay">
            <Button variant="hero" size="lg" className="mt-6 w-full"><CreditCard className="mr-2 h-5 w-5" /> Pay Now</Button>
          </Link>
        </motion.div>
          ...
          {(data?.payments || []).map((p: any) => (
            <div key={p.id} className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{new Date(p.created_at).toLocaleDateString()}</p>
