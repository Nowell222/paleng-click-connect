...
const CashierAcceptPayment = () => {
  ...
  const searchVendor = async () => {
    ...
    if (stalls?.length) {
      const { data: v } = await supabase.from("vendors").select("id, user_id").eq("stall_id", stalls[0].id).single();
      if (v) {
        const { data: profile } = await supabase.from("profiles").select("*").eq("user_id", v.user_id).single();
        setVendor({ ...v, stall: stalls[0], profile });
    ...
    if (profiles?.length) {
      const { data: v } = await supabase.from("vendors").select("id, user_id, stalls(*)").eq("user_id", profiles[0].user_id).single();
      if (v) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <div className="mt-6 flex gap-2">
          <Button onClick={() => { setDone(false); setVendor(null); setSearchTerm(""); }}>New Payment</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-lg space-y-6">
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-civic space-y-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder="Name or stall number..." className="h-11 pl-10 rounded-xl" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
        ...
        <>
          <div className="rounded-2xl border bg-card p-6 shadow-civic space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><p className="text-muted-foreground">Stall</p><p className="font-mono font-medium text-foreground">{vendor.stall?.stall_number}</p></div>
              <div><p className="text-muted-foreground">Section</p><p className="font-medium text-foreground">{vendor.stall?.section}</p></div>
